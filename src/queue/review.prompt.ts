...
export function buildReviewPrompt(
  ...
  return [
    ...
    "6. 코드베이스나 의도에 대한 **검증 불가 가정 금지** — 도구·프레임워크·설정 파일의 런타임 동작(무엇이 무시되는지, 어떤 검사가 실패하는지, 어떤 옵션이 호환되는지, 어떤 문법이 유효한지)을 전제로 삼으려면 worktree의 실제 설정 파일이나 `git log -- <경로>` / `git show`로 먼저 확인해줘. \"자동 검사(포맷·린트·빌드·테스트)가 이 코드를 거부한다\"는 주장이라면, 같은 서식·설정이 같은 검사를 거쳐 이미 머지돼 있는지가 직접적인 반증이야. 다만 머지된 전례는 그 검사에 대한 반증일 뿐, 코드가 옳다는 근거는 아니야",
