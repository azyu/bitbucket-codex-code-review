...
export function buildReviewPrompt(
  ...
  const diffSection = isBranchDiffMode
    ? [
        ...
        "",
        "```bash",
        `git merge-base refs/heads/${baseBranch} HEAD`,
        "git diff --no-ext-diff --find-renames --unified=80 <merge-base>..HEAD -- . ':(exclude,glob)**/pnpm-lock.yaml' ':(exclude,glob)**/package-lock.json' ':(exclude,glob)**/yarn.lock' ':(exclude,glob)**/bun.lockb'",
        "```",
        "",
