...
Bitbucket PR에 `@codex`를 멘션하거나 PR이 열리고 갱신될 때 코드 리뷰를 돌려, 인라인 코멘트와 요약을 PR에 남기는 워커 서비스입니다.
...
2. **트리거 감지** — 트리거 모드에 따라 자동(PR 오픈/업데이트) 또는 멘션(`@codex`)으로 리뷰 작업을 큐에 넣음
...
`pnpm build`는 `nest build`를 먼저 돌리고 대시보드의 `vite build`를 이어서 실행합니다. 순서를 고정한
이유는 `nest-cli.json`의 `deleteOutDir`가 `dist/`를 통째로 비우기 때문입니다 — 대시보드 산출물이
`dist/dashboard/`에 쌓이므로 `nest build`를 나중에 돌리면 함께 지워집니다.

같은 이유로 `pnpm start:dev`(`nest start --watch`)는 재컴파일할 때마다 `dist/dashboard/`를 날립니다.
그래서 대시보드 작업은 5173번 포트의 Vite dev server(`/api`는 `localhost:3000`으로 프록시)에서 하고,
합쳐진 결과는 `pnpm build && pnpm start`로 확인합니다.

`database:prepare`와 `migration:*`는 Nest `ConfigModule`을 거치지 않는 standalone TypeORM DataSource라
`process.env`만 봅니다. 그래서 package.json에서 Node의 `--env-file-if-exists=.env`로 `.env`를 직접 물려줍니다.
쉘이나 컨테이너에 이미 설정된 환경변수가 `.env` 값을 이기므로 Compose 배포 동작은 그대로입니다.

`.env.example`의 `REDIS_QUEUE_PORT`는 Compose가 호스트에 게시하는 `6381`입니다. 워커를 호스트에서 직접
띄울 때 이 값을 쓰고, Compose 안의 워커는 `redis:6379`를 그대로 씁니다.

대시보드는 `http://localhost:3000/dashboard`에서 열고, 잠금 화면에 `DASHBOARD_SECRET_KEY`를 입력합니다.
리뷰 이력 없이 화면만 보고 싶다면 데모 데이터를 넣으세요.
...
Compose는 `database:prepare`에서 `review_runs`가 없는 clean volume만 현재 schema로 초기화한 뒤 migration을 적용하고 worker를 띄웁니다. 기존 volume은 schema 동기화를 건너뛰고 migration만 실행하며, 실패하면 애플리케이션을 시작하지 않습니다. 애플리케이션의 `DB_SYNCHRONIZE`는 꺼져 있어 부팅 이후 schema는 바뀌지 않습니다.

개발용 Redis의 호스트 포트는 `127.0.0.1:6381`에만 게시됩니다. Compose 안의 worker는 기존처럼 `redis:6379`로 붙습니다. 이 설정은 외부 인프라 저장소의 운영 배포에는 적용되지 않습니다.

> [!IMPORTANT]
> Codex의 대형 PR branch-diff 모드는 worktree에서 `git`을 실행하는 sandbox를 씁니다. 이 sandbox의 bubblewrap이 비특권 user namespace를 만들 수 있도록 `code-review-worker`는 `seccomp:unconfined`로 실행해야 합니다. `CAP_SYS_ADMIN`이나 `privileged`까지는 필요 없습니다. 배포한 뒤 파드나 컨테이너에서 `unshare --user --map-root-user true`가 성공하는지 확인하세요.
...
나머지 리뷰·연동 설정은 최초 부팅 때 환경변수에서 MySQL로 한 번 이관된 다음부터는
`/dashboard`에서 관리합니다. Pod를 재시작하지 않아도 새 webhook과 job부터 적용됩니다.
...
> [!NOTE]
> `auto`·`both` 모드에서는 `pullrequest:updated` 이벤트도 처리합니다. 같은 commit hash를 두 번 리뷰하는 일은 idempotency key가 막아 줍니다.
> 같은 commit을 다시 리뷰하려면 트리거 모드와 관계없이 PR 댓글에 `@codex --force`를 입력하세요. 웹훅 재전송은 댓글 ID를 기준으로 걸러집니다.
...
Workspace와 repository 식별자는 점을 그대로 살리면서 대소문자를 구분하지 않는 파일시스템에서도 충돌하지 않도록 경로로 인코딩합니다. 기존 bare cache의 `origin`이 요청 저장소와 다르면 fetch 전에 실패하며, 캐시를 알아서 지우지는 않습니다. 이전 버전이 `foo.bar`를 `foobar.git`에 저장해 두었다면 관련 job을 drain한 뒤 해당 캐시의 `origin`을 확인하고 운영자가 직접 옮기거나 정리해야 합니다.
...
이 migration의 workspace-qualified idempotency key 변환은 되돌릴 수 없습니다. Migration을 적용한 뒤에 legacy 이미지로만 갈아 끼워 rollback해서는 안 됩니다. 호환되는 수정 이미지를 배포하거나, webhook ingress를 멈추고 새 형식 job을 모두 drain한 다음 배포 전 DB backup과 환경변수 설정을 함께 되돌려야 합니다.
여러 인스턴스가 동시에 시작하면 MySQL advisory lock이 schema 준비부터 migration ledger 기록까지 직렬화합니다. Idempotency key 변환은 별도 transaction marker와 함께 commit되고, 변환 도중 실패하면 둘 다 rollback되어 다음 실행이 다시 시도합니다.
...
- **Webhook 요청 제한** — 서명 형식·raw body·저장소 식별자를 먼저 검사하고, secret 조회 시도는 프로세스 전체에서 60초 고정 구간당 120회로 묶습니다. 형식만 맞고 서명이 틀린 요청도 횟수를 깎으며, 한도를 넘으면 DB를 건드리지 않고 `429`와 남은 초 단위 `Retry-After`를 돌려줍니다. 정상 요청도 같은 한도를 나눠 쓰므로 트래픽이 많거나 인스턴스가 여럿이면 ingress나 분산 제한을 따로 둬야 합니다.
...
repo 통계 응답에는 리뷰 건수, Codex 소요 시간과 전체 소요 시간, input·cached·output token 합계, 최신 리뷰 메타데이터가 담깁니다.

## Local Dashboard

내장 대시보드는 `GET /dashboard`에서 확인합니다. 잠금 화면에 `DASHBOARD_SECRET_KEY`를 입력하면 모든 `/api/internal/*` 조회·수정 요청에 Bearer header로 붙습니다. Key는 브라우저 메모리에만 남으므로 새로고침하거나 로그아웃하거나 401을 받으면 다시 입력해야 합니다. Runtime secret은 값 대신 configured/inherited 상태만 보여 줍니다.

잠금이 걸리거나 401이 나면 입력 중이던 전역·저장소 비밀값과 custom prompt도 함께 비웁니다. 이전 세션에서 시작한 비동기 응답은 다시 로그인한 세션의 상태를 건드리지 않습니다. 운영 환경의 `/api/internal/*` 네트워크 노출 정책은 외부 인프라에서 확인해야 하고, 지금까지 파악한 내용은 [#83](https://github.com/azyu/bitbucket-codex-code-review/issues/83)에 기록되어 있습니다.

### 구성

`dashboard/`는 Vite + Svelte 5 + TypeScript 패키지이자 pnpm workspace 멤버입니다. 산출물은 정적 파일이고
`dist/dashboard/`에 빌드되며, `main.ts`의 static mount 하나가 `/dashboard` prefix로 서빙합니다.
런타임 의존성은 없습니다 — 패키지의 의존성이 전부 devDependency라 `pnpm install --prod`는 아무것도
설치하지 않습니다.

`/dashboard` 아래 경로는 전부 완화된 CSP를 받습니다. Vite가 content hash가 붙은 파일명을 내보내므로
`src/dashboard-csp.ts`의 `isDashboardPath()` prefix 판정 하나로 문서와 asset을 함께 처리합니다
(정확 일치 목록을 두 곳에 중복해 두던 예전 구조를 대체했습니다). 현재 정책은 `script-src 'self'` /
`style-src 'self'`로, 모든 directive에서 helmet 기본값보다 엄격합니다.
...
최초 배포에서는 `OPENAI_API_KEY`와 HTTPS `OPENAI_BASE_URL` 환경변수가 runtime settings로 이관됩니다. 그다음부터는 대시보드에서 같이 관리하며, 새 job이 시작할 때 같은 DB revision에서 connection snapshot 하나로 읽습니다. Codex 실행에는 API key만 allowlist child env로 넘기고, CLI에는 `model_provider="openai"`와 `openai_base_url`을 명시합니다.

### `auth.json` 볼륨 마운트 대안

`codex login`으로 만들어지는 `~/.codex/auth.json`을 컨테이너의 `/root/.codex`에 마운트합니다. `docker-compose.yml`의 주석 처리된 볼륨 항목을 참고하세요.

대형 PR의 branch-diff 리뷰를 쓰려면 Codex bubblewrap이 비특권 user namespace를 만들 수 있어야 합니다. `docker-compose.yml`은 워커에 `security_opt: seccomp:unconfined`를 적용합니다. `CAP_SYS_ADMIN`이나 `privileged`까지는 필요 없습니다. 컨테이너 안에서 `unshare --user --map-root-user true`가 성공하는지 확인하세요.

> [!NOTE]
> `auth.json` mount는 bootstrap-static 대안입니다. 대시보드에 OpenAI API key를 넣어 두었다면 HTTPS endpoint도 대시보드 값이 Codex `config.toml`보다 우선합니다.
